                ...
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                    ...
                    <tr>
                        <td align="center" style="padding: 0px 0px 5px 0px; font-family: Lato, Helvetica, Arial, sans-serif; color: #4b4847; font-size: 14px;">
                        	...
                        	<input type="email" id="customer_email" class="custom-input" required placeholder="Enter Your Email">
                          <input  class="phone" id="customer_mobile" style="width: 100%; padding: 10px 10px 10px 73px; text-align: left;border-radius: 5px;border:1px solid #bbbbbb" type="tel"   required maxlength="19" placeholder=""/>
                          <input type="hidden" value="1" id="country_code" name="country_code">
                          <input type="hidden"  value="{{ number_format($payment->amount, 2, '.', ',') }}" id="transaction_amount">
                          <input type="hidden" value="{{$payment->currency}}" id="currency">
                          <input type="hidden" value="{{$payment->agentID}}" id="agentID">
                          <input type="button" onclick="initializeFoloosiPayment()" id="chargeButton"  style="margin-top: 10px;width: 100%;padding: 13px;background: #326bd1;border-radius: 30px;border: none;color: white;cursor: pointer;" value="Pay {{$payment->currency}}  {{ number_format($payment->amount, 2, '.', ',') }}">
                            </form>
                        </td>
                    </tr>
    ...
    .then(d => {
     ...
     foloosiHandler(response, function (e) {
        ...
        function handlePaymentSuccess(transaction_no,status) {
              var foloosiSuccessUrl = "{{ url('foloosi-success') }}";
              var agentID=$('#agentID').val();
let datas = {
    transaction_no: transaction_no || null,
    agentID:agentID
};
    fetch(`{{ url('save-Foloosipayment-data') }}`, {
        method: 'POST',
        headers: {
                     'Content-Type': 'application/json',
    ...
    .then(data => {
        ...
        window.location.href = foloosiSuccessUrl + '?status=' + encodeURIComponent(status);
                    // window.location.href = foloosiSuccessUrl + '?status=' + encodeURIComponent(status) + '&agentID=' + encodeURIComponent(agentID);
    })
    .catch((error) => {
