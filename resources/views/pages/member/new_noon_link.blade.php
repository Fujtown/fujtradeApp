                                ...
                                <form id="createLink" action="">
                                    ...
                                    <input type="hidden" value="" id="result">
                                     ...
                                     <div class="form-group row">
                                        ...
                                        <div class="col-sm-8">
                                           <input type="button" style="display:none;" class="btn btn-info generate" value="Generate">
                                        </div>
                                    </div>
 ...
 {{-- <script src="{{asset('admin_assets/js/share.js')}}"></script> --}}
  ...
  document.addEventListener('DOMContentLoaded', function () {
   ...
   document.getElementById('kycUpload').addEventListener('change', function() {
    ...
    fetch('{{ route("coffee.member.uploadKyc") }}', {
        method: 'POST',
        body: formData,
    })
    .then(response => {
    ...
    $(document).ready(function() {
       ...
       $('.generate').click(function() {
                ...
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                         ...
                         url: '{{ url('coffee/agent/store_noon_link') }}',
                         type: "POST",
                        data: data,
                        processData: false,
                        contentType: false,
...
$('#amount').on('blur', async function(){
    ...
    if(currency === 'USD') {
        let limit = 500;
        if(amount < limit) {
            alert('Amount should be greater than 500 USD');
            $(this).val('');
        }
    } else {
